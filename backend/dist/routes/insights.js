...
async function getCachedInsight(key) {
    const res = await pool.query('SELECT insight FROM insights_cache WHERE cache_key = $1 AND expires_at > NOW()', [key]);
    return res.rows[0]?.insight ?? null;
}
async function cacheInsight(key, insight) {
    const expiresAt = new Date();
    expiresAt.setMonth(expiresAt.getMonth() + 1); // 1 month TTL
    await pool.query('INSERT INTO insights_cache (cache_key, insight, expires_at) VALUES ($1, $2, $3) ON CONFLICT (cache_key) DO UPDATE SET insight = $2, expires_at = $3', [key, insight, expiresAt]);
}
// Real DB fetchers
const fetchMonthlyDataFromDB = async (month) => {
    // Tasks created this month from tasks_metadata
    const tasksRes = await pool.query(`SELECT COUNT(*) AS tasks_created FROM tasks_metadata WHERE TO_CHAR(created_at, 'YYYY-MM') = $1`, [month]);
    ...
    const statsRes = await pool.query(`
        ...
        FROM users
    `);
    const topRes = await pool.query('SELECT username FROM users ORDER BY current_score DESC LIMIT 1');
...
const fetchCreatorDataFromDB = async (address, _month) => {
    ...
    const rankRes = await pool.query('SELECT COUNT(*) + 1 AS rank FROM users WHERE current_score > $1', [user.current_score]);
    const totalRes = await pool.query('SELECT COUNT(*) AS total FROM users');
    // Score gap to the user directly above
    const aboveRes = await pool.query('SELECT current_score FROM users WHERE current_score > $1 ORDER BY current_score ASC LIMIT 1', [user.current_score]);
    ...
    return {
        ...
        totalStx: user.total_stx_funded,
        totalUsdcx: user.total_usdcx_funded,
...
const fetchPlatformAverages = async (_month) => {
...
router.get('/insights/general/:month', async (req, res) => {
    try {
...
router.get('/insights/personal/:address/:month', async (req, res) => {
    try {
