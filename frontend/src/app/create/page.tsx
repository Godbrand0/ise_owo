          ...
          <Card className="bg-orange-600/5 border-orange-600/20">
            ...
            <CardContent className="text-sm space-y-2">
              <div className="flex justify-between">
                ...
                <span>{formData.amountStx} STX</span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-400">Platform Base Fee (2%)</span>
                <span>{(Number(formData.amountStx) * 0.02).toFixed(2)} STX</span>
              </div>
              <div className="flex justify-between border-b border-zinc-800 pb-2">
                <span className="text-zinc-400">Optional Tip ({formData.tipPercent}%)</span>
                <span>{(Number(formData.amountStx) * (Number(formData.tipPercent) / 100)).toFixed(2)} STX</span>
              </div>
              <div className="flex justify-between font-bold text-base pt-1">
                <span>Total Escrow Required</span>
                <span className="text-orange-500">
                  {(Number(formData.amountStx) * (1 + 0.02 + Number(formData.tipPercent) / 100)).toFixed(2)} STX
                </span>
              </div>
            </CardContent>
          </Card>
